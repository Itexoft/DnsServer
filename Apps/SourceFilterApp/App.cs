...
public sealed class App : IDnsApplication, IDnsPostProcessor
{
    ...
    private Rule GetRule(string name, IPAddress clientIp)
    {
        Rule best = null;
        var bestScore = -1;
    ...
    public Task<DnsDatagram> PostProcessAsync(DnsDatagram request, IPEndPoint remoteEP, DnsTransportProtocol protocol, DnsDatagram response)
    {
        ...
        {
            var rule = this.GetRule(record.Name, clientIp);
            if (rule is null)
            {
                answer.Add(record);
            ...
            if (rule.PassesSplit(clientIp, record, response.Answer))
                answer.Add(record);
        }
    ...
    private sealed class Rule
    {
        ...
        public bool PassesSplit(IPAddress clientIp, DnsResourceRecord record, IReadOnlyList<DnsResourceRecord> answer)
        {
            if (this.split.Length == 0)
                return true;

            if (!TryGetAddress(record, out var recordIp))
                return true;

            var clientInside = this.ContainsSplitAddress(clientIp);
            var recordInside = this.ContainsSplitAddress(recordIp);
            ...
            return !this.HasSplitAddress(answer);
        }

        private bool HasSplitAddress(IReadOnlyList<DnsResourceRecord> answer)
        {
            foreach (var record in answer)
                if (TryGetAddress(record, out var ip) && this.ContainsSplitAddress(ip))
                    return true;

            return false;
        }

        private bool ContainsSplitAddress(IPAddress ip)
        {
            foreach (var sn in this.split)
                if (sn.Network.Contains(ip))
                    return true;

            return false;
        }

        private static bool TryGetAddress(DnsResourceRecord record, out IPAddress ip)
        {
            ip = record switch
            {
        ...
        {
            public SplitNetwork(NetworkAddress network)
            {
